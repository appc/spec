...
* **app** (object, optional) if present, defines the default parameters that can be used to execute this image as an application.
	...
	* **userAnnotations** (object, optional) map of arbitrary key-value data for end-user use. Values must be strings. Unlike Annotations, UserAnnotations MUST NOT affect runtime behavior and support arbitrary strings as keys.
	* **userLabels** (object, optional) map of arbitrary key-value data for end-user use. Values must be strings. UserLabels MUST NOT affect runtime behavior.
